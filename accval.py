...
with open(r'testrandom.pkl','rb') as f:
    data_dic = pk.load(f)
...
for k in range(1,10):
    ...
    acc = acc + max(test_acc_fold[k][test_acc_fold[(k)%10].index(max(test_acc_fold[(k)%10]))],np.array(val_acc_fold[(k-1)%10][val_acc_fold[(k-1)%10].index(max(val_acc_fold[(k-1)%10]))]))
    print(max(test_acc_fold[k][test_acc_fold[(k)%10].index(max(test_acc_fold[(k)%10]))],np.array(val_acc_fold[(k-1)%10][val_acc_fold[(k-1)%10].index(max(val_acc_fold[(k-1)%10]))])))
