...
fold_acc = []
for mm in range(3):
    with open(r'Huaximacc'+str(mm)+r'.pkl', 'rb') as f:
        data_dic = pk.load(f)
    train_acc_fold = data_dic['train']
    test_acc_fold = data_dic['test']
    val_acc_fold = data_dic['val']
    ...
    for k in range(1,10):
        ...
        for i in range(len(test_acc_fold[k])):
            if(val_acc_fold[(k)%10][i]== max(val_acc_fold[(k)%10])):
                max_list.append(i)
        for j  in range(len(max_list)):
            max_acc.append(test_acc_fold[k][max_list[j]])
        print (max(max_acc))
        acc = acc + sum(max_acc)/len(max_acc)
        temp_acc.append(sum(max_acc)/len(max_acc))
        #print(i,max_acc)
        ...
        max_acc = []
    fold_acc.append(temp_acc)
#print(fold_acc)
new_acc = 0.0
for k in range(9):
    print(k)
    new_acc = new_acc + max(max(fold_acc[0][k],fold_acc[1][k]),fold_acc[2][k])
print(new_acc/9)
tr_acc = tr_acc/10.0
te_acc = te_acc/10.0
vl_acc = vl_acc/10.0
